...
describe("GitHub Actions trust boundaries", () => {
  const pullRequest = workflows.find(
    (workflow) => workflow.file === "pull-request.yml",
  );
  const liveIntegration = workflows.find(
    (workflow) => workflow.file === "live-integration.yml",
  );
  ...
  it("uses a secret-free pull-request workflow and a trusted follow-up workflow", () => {
    ...
    assert.equal(
      hasTopLevelTrigger(pullRequest.source, "workflow_dispatch"),
      true,
    );
    ...
    assert.equal(
      hasTopLevelTrigger(pullRequest.source, "pull_request_target"),
      false,
    );
    assert.equal(
      hasTopLevelTrigger(liveIntegration.source, "workflow_run"),
      true,
    );
    assert.equal(
      hasTopLevelTrigger(liveIntegration.source, "pull_request"),
      false,
    );
    assert.equal(
      hasTopLevelTrigger(liveIntegration.source, "pull_request_target"),
      false,
    );
    assert.match(
      liveIntegration.source,
      /^    workflows: \[Pull request\]\r?$/m,
    );
    for (const file of [
      "quality.yml",
      "unit-tests.yml",
      ...
      assert.equal(
        workflows.some((workflow) => workflow.file === file),
        false,
      );
    }
  });
  ...
  it("runs quality and unit tests without secrets or an environment", () => {
    ...
    assert.match(
      getJob(pullRequest.source, "quality"),
      /^\s+- run: npm run typecheck\r?$/m,
    );
    assert.match(
      getJob(pullRequest.source, "quality"),
      /^\s+- run: npm run build\r?$/m,
    );
    assert.match(
      getJob(pullRequest.source, "quality"),
      /^\s+run: git diff --exit-code -- dist\r?$/m,
    );
    assert.match(
      getJob(pullRequest.source, "unit-tests"),
      /^\s+- run: npm test\r?$/m,
    );
  ...
  it("runs the exact approved commit only after the secret-free workflow succeeds", () => {
    ...
    assert.match(
      live,
      /test "\$\(git rev-parse HEAD\)" = "\$APPROVED_HEAD_SHA"/,
    );
  ...
  it("keeps protected environment access limited to live testing and session refresh", () => {
    assert.deepEqual(
      workflows
        .filter((workflow) =>
          /^\s+environment: Live Integration Tests\r?$/m.test(workflow.source),
        )
  ...
  it("retains the separate protected session refresh automation", () => {
    const refresh = workflows.find(
      (workflow) => workflow.file === "refresh-live-session.yml",
    );
    assert.ok(refresh);
    ...
    assert.match(
      refresh.source,
      /^\s+environment: Live Integration Tests\r?$/m,
    );
    assert.match(
      refresh.source,
      /^\s+run: npm run ci:reauth -- --refresh\r?$/m,
    );
    assert.match(refresh.source, /secrets\.TR_SESSION_JSON/);
    assert.match(
      refresh.source,
      /secrets\.GH_CLI_TOKEN_USED_TO_UPDATE_TR_SESSION/,
    );
